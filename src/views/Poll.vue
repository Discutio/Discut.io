			...
			<div class="content main" v-if="item.title">
				<div class="container">
					<div class="row">
                        ...
                        <span class="btn-action" @click="$modal.show('problem-introduce')">
                            ...
                            <span v-else-if="!mapOpened"> Show</span> <span v-if="1 == type"> discussion</span> <span
										v-else-if="2 == type"> poll</span>
                            map
                        </span>
				...
				<div class="container" style="position:relative;">
					<div class="row" v-if="mapOpened">
						<div class="col-lg-12">
							<div style="" class="map" v-if="1 == mode">
								...
								<div class="col-map">
									<div
											v-tooltip="{content: 'Current: ' + pollCurrentElement.title}"
											class="active element o-id-neutral-q"
											v-if="type == 2">
									</div>
									<div v-tooltip="{content: map.neutral.title}"
									     class="element o-id-neutral"
									     :class="{'active' : (1 === type || mode === 2), 'o-id-neutral-m' : pollCurrentElement}">
									</div>
								</div>
							...
							<div class="poll-map" v-if="2 == mode">
								<div style="" class="map">
									<div class="col-map">
								...
								<div class="map">
									<div style="clear:both;"></div>
									<div class="col-map">
										<div class="element" v-for="(_item, _key) in pollQuestions" v-tooltip="{ content: _item.title }"
										     :class="['o-id-horizontal-'+_key]">
										</div>
									</div>
								</div>
							</div>
					...
					<div class="row">
						<div class="col-lg-10 offset-1">
							...
							<div class="row">
								...
								<div class="col-lg-6" v-if="1 == mode">
									<div class="_main-box">
									...
									<div v-if="1 == mode" v-for="item in prosComments" class="_main-box">
                                    ...
                                    <span v-for="(reply, k) in item.replies" v-if="k <= 12" class="reply"
                                          :class="{'cons': reply.type == 'cons', 'pros': reply.type == 'pros'}">
											...
											<span v-if="item.replies.length > 12" class="reply with-content">
								...
								<div class="col-lg-6" v-if="1 == mode">
									...
									<div class="_main-box" v-if="1 == mode" v-for="item in consComments">
										<div class="main-box">
											<div>
                                        <span @click="changeCurrent(item)" class="comment-title" v-html="item.desc">
                                        </span>
								...
								<div class="col-lg-12" v-if="2 == mode">
									<div v-for="(item, i) in pollQuestions" class="_main-box">
										<div class="main-box" :class="{'first':0==i,'second':1==i,'third':2==i}">
											<div>
                                        <span @click="changeCurrent(item, false, item.title)" class="comment-title">
                                    ...
                                    <span v-for="(reply, k) in item.replies" v-if="k <= 24" class="reply"
                                          :class="{'cons': reply.type == 'cons', 'pros': reply.type == 'pros'}">
                                    </span>
											<span v-if="item.replies.length > 24" class="reply with-content">
                                        +{{item.replies.length-12}}
                                    </span>
										</div>
									</div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            changeCurrent: function(item, back = false, pollQuestion = "")
            {
                ...
                {
                    this.mode = 1;
                    this.firstClickInPoll = true;
                    this.pollCurrentElement = item;
                }

                if(this.type == 2)
                {
                    if(this.current.type == "origin")
                    {
                        this.mode = 2;
                        this.firstClickInPoll = false;
                        this.pollCurrentElement = {};
                    }
                }

                this.getComments(item.url, lastUrl, back, pollQuestion);
            ...
            getComments: function(url, oldUrl = "", back = false)
            {
                ...
                if(elementIndex === this.problem.author+"/"+this.problem.perm && back && this.type === 2)
                {
                    return;
                }
                ...
                {
                    elementIndex = this.current.permlink;

                    result = this.state[this.current.permlink];
                    replies = result;
                }else{
                    result = this.state[this.pollCurrentElement.title][elementIndex];
                    ...
                    steem.api.getContent(user, this.$route.params.slug, (err, result) =>
                    {
                        if(!err && result.id > 0)
                        {
                            if(result.json_metadata)
                            {
                                ...
                                this.map.neutral = {
                                    title: (1 === obj.type ? "Main discussion: " : "Poll: ") + this.item.title,
                                ...
                                steem.api.getState(result.url, (err, _result) =>
                                {
                                    if(!err)
                                    {
                                        ...
                                        {
                                            let html =
                                                "<html><h5>Which religion is the best?</h5><p><ul class='__dlist'><li><h4>Atheism</h4><p>Atheism is, in the broadest sense, the absence of belief in the existence of deities.[1][2][3][4] Less broadly, atheism is the rejection of belief that any deities exist.[5][6] <a href='https://en.wikipedia.org/wiki/Atheist_(disambiguation)'>In an even narrower sense, atheism is specifically</a> the position that there are no deities.[1][2][7][8] Atheism is contrasted with theism,[9][10] which, in its most general form, is the belief that at least one deity exists.[10][11][12]</p></li><li><h4>Islam</h4><p>Atheism is, in the broadest sense, the absence of belief in the existence of deities.[1][2][3][4] Less broadly, atheism is the rejection of belief that any deities exist.[5][6] <a href='https://en.wikipedia.org/wiki/Atheist_(disambiguation)'>In an even narrower sense, atheism is specifically</a> the position that there are no deities.[1][2][7][8] Atheism is contrasted with theism,[9][10] which, in its most general form, is the belief that at least one deity exists.[10][11][12]</p></li><li><h4>Catolic</h4><p>Atheism is, in the broadest sense, the absence of belief in the existence of deities.[1][2][3][4] Less broadly, atheism is the rejection of belief that any deities exist.[5][6] <a href='https://en.wikipedia.org/wiki/Atheist_(disambiguation)'>In an even narrower sense, atheism is specifically</a> the position that there are no deities.[1][2][7][8] Atheism is contrasted with theism,[9][10] which, in its most general form, is the belief that at least one deity exists.[10][11][12]</p></li><li><h4>Atheism</h4><p>Atheism is, in the broadest sense, the absence of belief in the existence of deities.[1][2][3][4] Less broadly, atheism is the rejection of belief that any deities exist.[5][6] <a href='https://en.wikipedia.org/wiki/Atheist_(disambiguation)'>In an even narrower sense, atheism is specifically</a> the position that there are no deities.[1][2][7][8] Atheism is contrasted with theism,[9][10] which, in its most general form, is the belief that at least one deity exists.[10][11][12]</p></li></ul></p></html>";

                                            let elem = $(html).filter("ul.__dlist").find("li");

                                            elem.each((i, v) => {
                                                this.pollQuestions.push(
                                                    {
                                                        title: $(v).find("h4").text(),
                                                        desc: $(v).find("p").html(),
                                                        votes: 0,
                                                        type: 0,
                                                        users: [],
                                                        author: "",
                                                        permlink: $(v).find("h4").text(),
                                                        replies: [
                                                            //{type: "cons"},
                                                            //{type: "pros"},
                                                        ],
                                                        stars: ["full", "full", "full", "blank", "blank"],
                                                        rewardHover: false,
                                                        isVoter: false,
                                                        busyVote: false,
                                                    }
                                        ...
                                        if(this.type === 1)
                                        {
                                            ...
                                            $.each(_result.content, (i, v) =>
                                            {
                                                ...
                                                let html =
                                                    "<html>Voted for <ul class='__dlist'><li>Islam</li></ul></html>";
                                                ...
                                                if(typeof pollContent[elem] == "undefined")
                                                {
                                                    return true;
                                                }

                                                pollContent[elem][i] = v;
                                            });
...
<style lang="scss">
	...
	.topic {
		...
		.main-box {
			...
			&.third {
				border-color: rgba(150,90,56, 0.6);
				.comment-title strong {
					color: rgba(150,90,56, 1);
				}
			}
			&.weight {
				padding-bottom: 30px;
